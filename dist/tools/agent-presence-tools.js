...
import { isPresenceEnabled, announcePresence, listPresence, leavePresence, reapStale, DEFAULT_PRESENCE_TTL_SECONDS, MAX_PRESENCE_TTL_SECONDS, } from '../lib/agent-presence.js';
...
function formatPresence(e) {
    const bits = [`- **${e.agent}** (${e.provider})`];
    if (e.role)
        bits.push(`role=${e.role}`);
    if (e.briefId)
        bits.push(`brief=${e.briefId}`);
...
// ---------------------------------------------------------------------------
export const agentPresenceAnnounceTool = {
    ...
    handler: async (args) => {
        ...
        try {
            ...
            return { content: [{ type: 'text', text: `Presence announced.\n\n${formatPresence(entry)}` }] };
        }
        catch (error) {
...
// agent_presence_list
// ---------------------------------------------------------------------------
export const agentPresenceListTool = {
    ...
    handler: async (args) => {
        ...
        try {
            ...
            if (entries.length === 0) {
                return { content: [{ type: 'text', text: `No live presence in "${args.namespace}".` }] };
            }
            const body = entries.map(formatPresence).join('\n');
            return { content: [{ type: 'text', text: `## Presence: ${args.namespace}\n\n${entries.length} ${entries.length === 1 ? 'agent' : 'agents'} present:\n\n${body}` }] };
        }
        catch (error) {
...
// ---------------------------------------------------------------------------
export const agentPresenceLeaveTool = {
    ...
    handler: async (args) => {
        ...
        try {
            ...
            if (!removed) {
                return { content: [{ type: 'text', text: `Agent "${args.agent}" was not present in "${args.namespace}".` }] };
            }
            return { content: [{ type: 'text', text: `Agent "${args.agent}" left "${args.namespace}".` }] };
        }
        catch (error) {
...
// agent_presence_reap
// ---------------------------------------------------------------------------
export const agentPresenceReapTool = {
    ...
    handler: async (args) => {
        ...
        try {
            ...
            return { content: [{ type: 'text', text: `Reaped ${result.removed} stale presence ${result.removed === 1 ? 'entry' : 'entries'}.` }] };
        }
        catch (error) {
...
//# sourceMappingURL=agent-presence-tools.js.map