...
import { isSharedContextEnabled, postEntry, readFeed, clearFeed, listContextNamespaces, digestChannel, listOpenQuestions, CONTEXT_KINDS, DEFAULT_READ_LIMIT, MAX_READ_LIMIT, } from '../lib/shared-context.js';
// ---------------------------------------------------------------------------
// Helpers
// ---------------------------------------------------------------------------
...
// shared_context_post
// ---------------------------------------------------------------------------
export const sharedContextPostTool = {
    ...
    schema: {
        ...
        kind: z.enum(CONTEXT_KINDS).default('note').describe('Entry kind: note, decision, finding, blocker, handoff, question, or answer'),
    ...
    handler: async (args) => {
        if (!isSharedContextEnabled())
            return disabledResponse();
        try {
            const root = validateWorkingDirectory(args.workingDirectory);
            const entry = postEntry(args.namespace, args.author, args.kind, args.message, { tags: args.tags, refs: args.refs }, root);
            const text = [
...
// shared_context_read
// ---------------------------------------------------------------------------
export const sharedContextReadTool = {
    ...
    schema: {
        ...
        kind: z.enum(CONTEXT_KINDS).optional().describe('Only entries of this kind'),
    ...
    handler: async (args) => {
        ...
        try {
            const root = validateWorkingDirectory(args.workingDirectory);
            const entries = readFeed(args.namespace, {
                limit: args.limit,
                kind: args.kind,
                author: args.author,
                since: args.since,
...
// shared_context_list
// ---------------------------------------------------------------------------
export const sharedContextListTool = {
...
// ---------------------------------------------------------------------------
export const sharedContextClearTool = {
...
// ---------------------------------------------------------------------------
export const sharedContextDigestTool = {
    ...
    handler: async (args) => {
        ...
        try {
            ...
            const kindLines = Object.entries(digest.byKind)
                .filter(([, n]) => n > 0)
                .map(([k, n]) => `  - ${k}: ${n}`)
                .join('\n');
...
// ---------------------------------------------------------------------------
export const sharedContextOpenQuestionsTool = {
