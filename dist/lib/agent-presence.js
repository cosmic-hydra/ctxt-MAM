 ...
 */
export function announcePresence(args, worktreeRoot) {
    ...
    if (ttl > MAX_PRESENCE_TTL_SECONDS)
        ttl = MAX_PRESENCE_TTL_SECONDS;
    ...
    if (args.briefId !== undefined)
        validateIdentifier(args.briefId, 'agent'); // same charset
    ...
    if (args.role !== undefined && args.role.length > 0)
        entry.role = args.role;
    if (args.focus !== undefined && args.focus.length > 0)
        entry.focus = args.focus;
    if (args.briefId !== undefined)
        entry.briefId = args.briefId;
 ...
 */
export function listPresence(namespace, opts = {}, worktreeRoot) {
    validateIdentifier(namespace, 'namespace');
    const dir = getNamespaceDir(namespace, worktreeRoot);
    if (!existsSync(dir))
        return [];
...
export function leavePresence(namespace, agent, worktreeRoot) {
...
export function reapStale(worktreeRoot) {
