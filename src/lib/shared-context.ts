 ...
 * shared memory).
 */
export function isSharedContextEnabled(): boolean {
  try {
...
function getFeedPath(namespace: string, worktreeRoot?: string): string {
...
function ensureContextDir(worktreeRoot?: string): string {
  const omcRoot = getOmcRoot(worktreeRoot);
...
function parseFeed(filePath: string): ContextEntry[] {
...
export function countEntries(namespace: string, worktreeRoot?: string): number {
 ...
 */
export function clearFeed(namespace: string, worktreeRoot?: string): { removed: number } {
  const filePath = getFeedPath(namespace, worktreeRoot);
...
export function digestChannel(
  ...
  const tailByKind = (k: ContextKind): ContextEntry[] =>
    entries.filter(e => e.kind === k).slice(-limit).reverse();
 ...
 */
export function listOpenQuestions(namespace: string, worktreeRoot?: string): ContextEntry[] {
 ...
 */
export function listContextNamespaces(worktreeRoot?: string): ContextNamespaceSummary[] {
  const omcRoot = getOmcRoot(worktreeRoot);
  const dir = join(omcRoot, SHARED_CONTEXT_DIR);
  ...
  const summaries: ContextNamespaceSummary[] = [];
