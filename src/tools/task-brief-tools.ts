...
function renderBrief(brief: TaskBrief): string {
  const lines: string[] = [];
  ...
  if (brief.successCriteria.length > 0) {
    ...
    for (const sc of brief.successCriteria) lines.push(`- [ ] ${sc}`);
    lines.push('');
  }
...
export const taskBriefCreateTool: ToolDefinition<{
  briefId: z.ZodString;
  ...
  handler: async (args) => {
    if (!isTaskBriefEnabled()) return disabledResponse();
    try {
      ...
      return { content: [{ type: 'text' as const, text: `Created task brief.\n\n${renderBrief(brief)}` }] };
    } catch (error) {
...
export const taskBriefGetTool: ToolDefinition<{
  briefId: z.ZodString;
  namespace: z.ZodString;
  workingDirectory: z.ZodOptional<z.ZodString>;
}> = {
  ...
  handler: async (args) => {
    ...
    try {
      ...
      if (!brief) {
        return { content: [{ type: 'text' as const, text: `Brief "${args.briefId}" not found in namespace "${args.namespace}".` }] };
      }
      return { content: [{ type: 'text' as const, text: renderBrief(brief) }] };
    } catch (error) {
...
export const taskBriefUpdateStatusTool: ToolDefinition<{
  briefId: z.ZodString;
  namespace: z.ZodString;
  ...
  schema: {
    ...
    namespace: z.string().min(1).max(128).describe('Channel/team the brief lives in'),
    status: z.enum(TASK_STATUSES as unknown as [string, ...string[]]).describe('New status'),
  ...
  handler: async (args) => {
    ...
    try {
      ...
      const brief = updateStatus({
        namespace: args.namespace,
        briefId: args.briefId,
        status: args.status as TaskBrief['status'],
        by: args.by,
...
export const taskBriefAmendTool: ToolDefinition<{
  briefId: z.ZodString;
  namespace: z.ZodString;
  by: z.ZodString;
  ...
  handler: async (args) => {
    ...
    try {
      ...
      return { content: [{ type: 'text' as const, text: `Amended.\n\n${renderBrief(brief)}` }] };
    } catch (error) {
...
export const taskBriefListTool: ToolDefinition<{
  namespace: z.ZodString;
  ...
  schema: {
    ...
    status: z.enum(TASK_STATUSES as unknown as [string, ...string[]]).optional().describe('Filter by status'),
  ...
  handler: async (args) => {
    ...
    try {
      ...
      const briefs = listBriefs(args.namespace, {
        status: args.status as TaskBrief['status'] | undefined,
        owner: args.owner,
        tag: args.tag,
      }, root);
      if (briefs.length === 0) {
        return { content: [{ type: 'text' as const, text: `No briefs in namespace "${args.namespace}" matching filters.` }] };
      }
      const lines = briefs.map(b =>
        `- **${b.briefId}** [${b.status}] — ${b.title} (owners: ${b.owners.join(', ') || '_unassigned_'}, updated: ${b.updatedAt})`
      );
      return { content: [{ type: 'text' as const, text: `## Task Briefs: ${args.namespace}\n\n${briefs.length} ${briefs.length === 1 ? 'brief' : 'briefs'}:\n\n${lines.join('\n')}` }] };
    } catch (error) {
...
export const taskBriefDeleteTool: ToolDefinition<{
  briefId: z.ZodString;
  namespace: z.ZodString;
  workingDirectory: z.ZodOptional<z.ZodString>;
}> = {
  ...
  handler: async (args) => {
    ...
    try {
      ...
      if (!removed) {
        return { content: [{ type: 'text' as const, text: `Brief "${args.briefId}" not found in "${args.namespace}".` }] };
      }
      return { content: [{ type: 'text' as const, text: `Deleted brief "${args.briefId}".` }] };
    } catch (error) {
