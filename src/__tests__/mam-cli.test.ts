...
function capture(argv: string[]): { stdout: string; stderr: string; code: number } {
...
describe('omc-mam CLI', () => {
  ...
  describe('presence lifecycle', () => {
    it('announce + list + leave works for cross-provider agents', () => {
      ...
      expect(entries.map((e: { agent: string }) => e.agent).sort()).toEqual(['codex-1', 'gemini-2']);
      expect(entries.find((e: { agent: string }) => e.agent === 'codex-1').provider).toBe('codex');
      expect(entries.find((e: { agent: string }) => e.agent === 'gemini-2').provider).toBe('gemini');
