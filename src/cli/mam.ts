...
import {
  writeEntry as smWriteEntry,
  readEntry as smReadEntry,
  listEntries as smListEntries,
  deleteEntry as smDeleteEntry,
  listNamespaces as smListNamespaces,
} from '../lib/shared-memory.js';

// ---------------------------------------------------------------------------
// Minimal arg parser — keeps the CLI dependency-free and easy to bundle.
// ---------------------------------------------------------------------------
...
function out(parsed: ParsedArgs, structured: unknown, human: string): void {
...
function contextCmd(action: string, parsed: ParsedArgs): void {
  switch (action) {
    case 'post': {
      ...
      const kindArg = (optionalFlag(parsed, 'kind') ?? 'note') as ContextKind;
    ...
    case 'read': {
      ...
      const kindArg = optionalFlag(parsed, 'kind') as ContextKind | undefined;
...
function briefCmd(action: string, parsed: ParsedArgs): void {
  switch (action) {
    case 'create': {
    ...
    case 'status': {
      const brief = updateStatus({
        namespace: requireFlag(parsed, 'namespace'),
        briefId: requireFlag(parsed, 'id'),
        status: requireFlag(parsed, 'status') as TaskStatus,
    ...
    case 'list': {
      const items = listBriefs(requireFlag(parsed, 'namespace'), {
        status: optionalFlag(parsed, 'status') as TaskStatus | undefined,
...
function presenceCmd(action: string, parsed: ParsedArgs): void {
  switch (action) {
    case 'announce': {
...
function memoryCmd(action: string, parsed: ParsedArgs): void {
  switch (action) {
    case 'write': {
      const namespace = requireFlag(parsed, 'namespace');
...
export function runMam(argv: string[]): number {
